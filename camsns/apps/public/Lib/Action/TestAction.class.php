...
class TestAction extends Action{
	...
	// ALTER TABLE `ts_attach` ADD COLUMN `width`  int(11) NULL DEFAULT 0 COMMENT '图片宽度' AFTER `from`;
	// ALTER TABLE `ts_attach` ADD COLUMN `height`  int(11) NULL DEFAULT 0 COMMENT '图片高度' AFTER `width`;
	public function up_attach_image_location () {
		$p = isset($_GET['p']) ? intval($_GET['p']) : 1;
		$res = model('Attach')->upImageAttach($p, 100);
		if ($res) {
			$p++;
			echo '<script type="text/javascript">location.href="'.U('public/Test/up_attach_image_location', array('p'=>$p)).'"</script>';
		} else {
			exit('附件图片宽度与高度刷新完毕');
		}
	}

	public function up_attach_image_cloud () {
		$p = isset($_GET['p']) ? intval($_GET['p']) : 1;
		$res = model('Attach')->upImageAttachCloud($p, 100);
		if ($res) {
			$p++;
			echo '<script type="text/javascript">location.href="'.U('public/Test/up_attach_image_cloud', array('p'=>$p)).'"</script>';
		} else {
			exit('附件图片宽度与高度刷新完毕');
		}
	}
