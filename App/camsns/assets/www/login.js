...
$(document).ready(function() { 
    //导入phonegap库
  //自动登录
    console.log("是否登录"+U('public/Passport/isLogin'));
    $.post(U('public/Passport/isLogin'), {}, function(res) {
        if(res.status == 1) {
          ui.success(res.info);
           if(res.data.home_url==0){
                window.location.href = U('public/Index/index');  
            }else{
                window.location.href = res.data.home_url;            
            }
...
//window.plugins.JPushUtil.SetAlias("ethanzhu",function(data){alert("success");},function(e){alert(e);});
    $('#ajax_login_form').submit(function(){
        //window.plugins.JPushUtil.SetAlias("ethanzhu1111",function(data){alert("success");},function(e){alert(e);});
         $(this).ajaxSubmit({
    ...
    var loginCallback = function(i) {
        ...
        if(i.status==1){
         ...
         //设置别名，jpush推送
            console.log(""+i.data.uid);
            window.plugins.JPushUtil.SetAlias(""+i.data.uid,null,function(e){alert(e);});
            if(i.data.home_url==0){
                window.location.href = U('public/Index/index');  
            }else{
                window.location.href = i.data.home_url;            
            }
        }else{
            $('#js_login_input').html('<p>'+i.info+'</p>').show();
        }
